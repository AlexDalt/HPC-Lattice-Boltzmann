...
float comp_func(const t_param params, cl_mem* cells, cl_mem* tmp_cells, t_ocl ocl, int tot_cells){
  ...
  err = clSetKernelArg(ocl.comp_func, 2, NSPEEDS * size * sizeof(float), NULL);
  ...
  err = clSetKernelArg(ocl.comp_func, 7, sizeof(cl_int), &size);
  checkError(err, "setting comp_func arg 7", __LINE__);
  err = clSetKernelArg(ocl.comp_func, 8, sizeof(cl_float), &params.omega);
