...
int main(int argc, char* argv[])
{
  ...
  #pragma omp parallel private(ii,jj,kk)
  {
    ...
    #pragma omp for
      ...
      const double wi0 = 4.0 / 9.0;  /* weighting factor */
      const double wi1 = 1.0 / 9.0;  /* weighting factor */
      const double wi2 = 1.0 / 36.0; /* weighting factor */
          ...
          for (int a = ii; a < ii+STEP_COL && a < params.ny; a++){
            for (int b = jj; b < jj+STEP_COL && b < params.nx; b++){
              ...
              if (!obstacles[a * params.nx + b])
              {
                ...
                d_equ[0] = wi0 * local_density
                           * (1.0 - u_sq / (2.0 * c_sq));
                /* axis speeds: weight w1 */
                d_equ[1] = wi1 * local_density * (1.0 + u[1] / c_sq
                                                 + (u[1] * u[1]) / (2.0 * c_sq * c_sq)
                                                 - u_sq / (2.0 * c_sq));
                d_equ[2] = wi1 * local_density * (1.0 + u[2] / c_sq
                                                 + (u[2] * u[2]) / (2.0 * c_sq * c_sq)
                                                 - u_sq / (2.0 * c_sq));
                d_equ[3] = wi1 * local_density * (1.0 + u[3] / c_sq
                                                 + (u[3] * u[3]) / (2.0 * c_sq * c_sq)
                                                 - u_sq / (2.0 * c_sq));
                d_equ[4] = wi1 * local_density * (1.0 + u[4] / c_sq
                                                 + (u[4] * u[4]) / (2.0 * c_sq * c_sq)
                                                 - u_sq / (2.0 * c_sq));
                /* diagonal speeds: weight w2 */
                d_equ[5] = wi2 * local_density * (1.0 + u[5] / c_sq
                                                 + (u[5] * u[5]) / (2.0 * c_sq * c_sq)
                                                 - u_sq / (2.0 * c_sq));
                d_equ[6] = wi2 * local_density * (1.0 + u[6] / c_sq
                                                 + (u[6] * u[6]) / (2.0 * c_sq * c_sq)
                                                 - u_sq / (2.0 * c_sq));
                d_equ[7] = wi2 * local_density * (1.0 + u[7] / c_sq
                                                 + (u[7] * u[7]) / (2.0 * c_sq * c_sq)
                                                 - u_sq / (2.0 * c_sq));
                d_equ[8] = wi2 * local_density * (1.0 + u[8] / c_sq
